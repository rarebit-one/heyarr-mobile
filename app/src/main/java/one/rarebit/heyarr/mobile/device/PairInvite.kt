 ...
 *
 * The invite is the `voidbind:pair?v=2&relay=…&session=…&salt=…` tuple that
 * `voidbind pair-initiate` prints (as a QR) on the machine holding the user identity;
 * this phone joins it as the NEW device. Validation is the library's own
...
sealed interface PairInvite {
    /** A well-formed invite, whitespace-trimmed, ready for `DevicePairing.begin`. */
    data class Valid(val inviteQr: String, val relay: String, val session: String) : PairInvite
    ...
    companion object {
        ...
        fun check(raw: String): PairInvite {
            ...
            if (!text.startsWith(PAIR_PREFIX)) {
                return Invalid(
                    "Not a Voidbind pairing invite (saw \"${peek(text)}\"). Expected " +
                        "voidbind:pair?v=2&relay=…&session=…&salt=…",
                )
            }
            val parsed = try {
                Invite.decode(text)
            ...
            return Valid(inviteQr = text, relay = parsed.relay, session = parsed.session)
        }
